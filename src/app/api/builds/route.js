...
async function buildRealApk(project, nativeFeatures, logStep) {
  ...
  const orientation =
    nativeFeatures.orientation === "portrait"
      ? "portrait"
      : nativeFeatures.orientation === "landscape"
      ? "landscape"
      : "default";
...
async function buildSimulated(project, nativeFeatures, build, logStep) {
  const enabledFeatures = ["push", "camera", "location", "biometrics"].filter(
    (k) => nativeFeatures[k] === true
  );

  const steps = [
    { log: "Android SDK 33 / Gradle 8.2.1 wird initialisiert…", delay: 1200 },
    { log: `PWA-Manifest wird abgerufen von ${project.pwa_url}…`, delay: 1000 },
    { log: "AndroidManifest.xml wird generiert…", delay: 700 },
    {
      log: `  → Theme: ${nativeFeatures.theme_color || "#2563eb"} / ${nativeFeatures.orientation || "any"}`,
      delay: 300,
    },
    ...(enabledFeatures.length > 0
      ? [{ log: `Native Berechtigungen: ${enabledFeatures.join(", ")}`, delay: 500 }]
      : []),
    {
      log: `${project.build_type === "twa" ? "Trusted Web Activity" : "WebView"} Bridge konfigurieren…`,
      delay: 900,
    },
    { log: "App-Icons für alle Dichten verarbeiten…", delay: 800 },
    { log: "  → mdpi / hdpi / xhdpi / xxhdpi / xxxhdpi", delay: 400 },
    { log: "Gradle-Abhängigkeiten auflösen…", delay: 1800 },
    { log: "AAPT2 Ressourcen kompilieren…", delay: 1500 },
    { log: "Java-Quellen kompilieren…", delay: 1200 },
    { log: "D8 DEX-Compiler ausführen…", delay: 1000 },
    { log: "APK verknüpfen…", delay: 700 },
    { log: "APK mit Debug-Keystore signieren…", delay: 900 },
    { log: "zipalign-Optimierung…", delay: 500 },
  ];
  ...
  throw new Error(
    "Gradle Build-Fehler: Abhängigkeit androidx.browser:browser:1.6.0 konnte nicht aufgelöst werden"
  );
}
      ...
      try {
        ...
        await sql`
          ...
          SET status = 'completed',
              ...
              log_text = log_text || ${
                `\n\n✓ BUILD ERFOLGREICH` +
                (usedRealBuild ? " (Echter PWABuilder-Build)" : " (Simuliert)") +
