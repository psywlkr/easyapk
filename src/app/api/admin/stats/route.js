...
export async function GET(request) {
  try {
    ...
    const [usersResult, projectsResult, buildsResult, buildStatusResult] =
      await Promise.all([
        sql`SELECT COUNT(*) AS total FROM users`,
        sql`SELECT COUNT(*) AS total FROM projects`,
        sql`SELECT COUNT(*) AS total FROM builds`,
        sql`SELECT status, COUNT(*) AS count FROM builds GROUP BY status ORDER BY status`,
      ]);
